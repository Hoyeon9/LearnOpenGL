...
int main() {
	...
	cout << "normal\n";
	unsigned int metallicMap = loadTexture((mapPath + "/metallic.png").c_str());
	cout << "metallic\n";
	unsigned int roughnessMap = loadTexture((mapPath + "/roughness.png").c_str());
	cout << "roughness\n";
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//glUniform3fv(glGetUniformLocation(renderProgram, "F0"), 1, glm::value_ptr(glm::vec3(0.4f)));
		//glUniform1f(glGetUniformLocation(renderProgram, "metallic"), 1.0f);
		//glUniform1f(glGetUniformLocation(renderProgram, "roughness"), 0.0f);
		...
		for (int row = 0; row < 7; ++row) {
			glUniform1f(glGetUniformLocation(renderProgram, "metallic"), (float)row/nrRows);
			for (int col = 0; col < nrCols; ++col) {
				glUniform1f(glGetUniformLocation(renderProgram, "roughness"), glm::clamp((float)col / nrCols, 0.05f, 1.0f));
				model = glm::mat4(1.0f);
				...
				glUniformMatrix4fv(glGetUniformLocation(renderProgram, "model"), 1, GL_FALSE, glm::value_ptr(model));
				glUniformMatrix4fv(glGetUniformLocation(renderProgram, "mvp"), 1, GL_FALSE, glm::value_ptr(projection *view *model));
				glUniformMatrix4fv(glGetUniformLocation(renderProgram, "normalMatrix"), 1, GL_FALSE, glm::value_ptr(glm::transpose(glm::inverse(glm::mat3(model)))));
				glBindVertexArray(sphereVAO);
				glDrawElements(GL_TRIANGLE_STRIP, indexCount, GL_UNSIGNED_INT, 0);
			}
		}*/
