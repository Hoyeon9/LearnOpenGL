...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "Advanced1_stencil", NULL, NULL);
	if (window == NULL) {
	...
	//Loading shaders--------------------------
	unsigned int shaderProgram = loadShader("verA1.vs", "fragA1.fs");
	unsigned int stencilShader = loadShader("verA1.vs", "outline.fs");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		modelLoc = glGetUniformLocation(stencilShader, "model");
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		mvpLoc = glGetUniformLocation(stencilShader, "mvp");
		glUniformMatrix4fv(mvpLoc, 1, GL_FALSE, glm::value_ptr(projection * view * model));
