...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "Advanced7", NULL, NULL);
	if (window == NULL) {
	...
	//Loading shaders--------------------------
	unsigned int shaderProgram = loadShader("verA7.vs", "fragA7.fs");
	...
	unsigned int floor = loadTexture("wood.jpg");
	unsigned int floorGamma = loadTexture("wood.png");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		unsigned int lightColLoc = glGetUniformLocation(shaderProgram, "lightColors");
		glUniform3fv(lightColLoc, 4, &lightColors[0][0]);
		unsigned int lightPosLoc = glGetUniformLocation(shaderProgram, "lightPositions");
		glUniform3fv(lightPosLoc, 4, &lightPositions[0][0]);
		unsigned int viewPosLoc = glGetUniformLocation(shaderProgram, "viewPos");
		glUniform3fv(viewPosLoc, 1, glm::value_ptr(cameraPos));

		glUniform1f(glGetUniformLocation(shaderProgram, "gamma"), gammaEnabled);

		glBindVertexArray(planeVAO);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, gammaEnabled ? floorGamma : floor);
...
void processInput(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS && !gammaKeyPressed)
	{
