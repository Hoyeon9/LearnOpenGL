...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "Advanced4", NULL, NULL);
	if (window == NULL) {
	...
	//Loading shaders--------------------------
	unsigned int shaderProgram = loadShader("verA4.vs", "fragA4.fs");
	unsigned int skyboxShader = loadShader("skybox.vs", "skybox.fs");
	unsigned int reflectShader = loadShader("reflect.vs", "reflect.fs");
	...
	vector<std::string> cubeTexFaces{
		"skybox/right.jpg",
		"skybox/left.jpg",
		"skybox/top.jpg",
		"skybox/bottom.jpg",
	...
	glUniform1i(glGetUniformLocation(reflectShader, ("skybox")), 0);
	...
	for (int i = 0; i < cubeTexFaces.size(); i++) {
		unsigned char* data = stbi_load(cubeTexFaces[i].c_str(), &width, &height, &nrChannels, 0);
		if (data) {
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		}
		else {
			std::cout << "Cubemap texture failed to load at path: " << cubeTexFaces[i] << std::endl;
		}
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUseProgram(reflectShader);
		glUniform3fv(glGetUniformLocation(reflectShader, ("cameraPos")), 1, glm::value_ptr(cameraPos));
		unsigned int vpLoc = glGetUniformLocation(reflectShader, "vp");
		glUniformMatrix4fv(vpLoc, 1, GL_FALSE, glm::value_ptr(projection * view));
		unsigned int modelLoc = glGetUniformLocation(reflectShader, "model");
		...
		glUseProgram(skyboxShader);
		unsigned int mvpLocSky = glGetUniformLocation(skyboxShader, "vp");
		glUniformMatrix4fv(mvpLocSky, 1, GL_FALSE, glm::value_ptr(projection * glm::mat4(glm::mat3(view))));
		glBindVertexArray(skyboxVAO);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_CUBE_MAP, skyboxTexture);
		glDrawArrays(GL_TRIANGLES, 0, 36);
