...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "Advanced2_Blending", NULL, NULL);
	if (window == NULL) {
	...
	//Loading shaders--------------------------
	unsigned int shaderProgram = loadShader("verA2.vs", "fragA2.fs");
   ...
   // Right face
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for (int i = 0; i < glasses.size(); i++) {
			float distance = glm::length(cameraPos - glasses[i]);
