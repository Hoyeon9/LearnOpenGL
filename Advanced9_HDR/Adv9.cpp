...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "Advanced9", NULL, NULL);
	if (window == NULL) {
	...
	//Loading shaders--------------------------
	unsigned int shaderProgram = loadShader("verA9.vs", "fragA9.fs");
	unsigned int quadProgram = loadShader("quad.vs", "quad.fs");
	...
	glm::vec3 lightPos[] = {
		...
		glm::vec3 (0.6f, 0.7f, 1.3f),
		glm::vec3 (1.1f, -0.3f, -0.0f),
		glm::vec3 (0.8f, -1.0f, 2.0f)
	};
	glm::vec3 lightcolor[] = {
	...
	glUniform1i(glGetUniformLocation(shaderProgram, ("wood")), 0);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUniform3fv(lightColLoc, 4, &lightcolor[0][0]);
		unsigned int lightPosLoc = glGetUniformLocation(shaderProgram, "lightPositions");
		glUniform3fv(lightPosLoc, 4, &lightPos[0][0]);
		...
		glUniform3fv(glGetUniformLocation(shaderProgram, "light.ambientStrength"), 1, glm::value_ptr(glm::vec3(0.1f)));
		...
		glUniform1f(glGetUniformLocation(quadProgram, "exposure"), 0.5f);
		glBindVertexArray(quadVAO);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, colorBuffer);
